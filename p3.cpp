...
void draw_circle3(int xc, int yc, int x, int y)
{
	glBegin(GL_POINTS);
...
void draw_cylinder()
{
	...
	display_cylinder(xc3, yc3, r3);
	glColor3f(1, 0, 0);
	for (y = yc3 + 1; y < 300; y++)
		display_cylinder(xc3, y, r3);

	glColor3f(0, 0, 0);
	for (r1 = 0; r1 <= r3; r1++)
		display_cylinder(xc3, y, r1);
...
void draw_parallelopiped()
{
	int y;
	glClear(GL_COLOR_BUFFER_BIT);
	//back face of the parallelelopiped
	glColor3f(0, 0, 1);
	...
	glColor3f(0, 0, 1);
	//front face of the parallelelopiped
	glBegin(GL_POLYGON);
	glVertex2i(point[0][0]--, point[0][1]--);
	glVertex2i(point[1][0]--, point[1][1]--);
	glVertex2i(point[2][0]--, point[2][1]--);
...
void p3_main(int argc, char **argv)
{
